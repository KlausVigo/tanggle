---
title: "***tanggle***: Visualization of phylogenetic networks in a *ggplot2* framework (bioc)"
author:
- name: Klaus Schliep
...
abstract: |
  An R package for visualization of phylogenetic networks in a *ggplot2* 
  framework
vignette: |
  %\VignetteIndexEntry{***tanggle***: Visualization of phylogenetic networks in a *ggplot2* framework (bioc)}
...
:-------- | :--------------------------------------------|
`geom_splitnet` | Adds a *splitnet* layer to a ggplot, to combine visualising 
...
`minimize_overlap` | Reduces the number of reticulation lines crossing over in 
the plot
`node.depth.evonet` | Returns the depths or heights of nodes and tips in the 
